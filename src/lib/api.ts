...
if (isBrowser) {
  try {
    ...
    if (saved) {
      ...
      globalSettings = { ...globalSettings, ...parsed, fourAnimo: parsed.fourAnimo === true };
    } else {
      globalSettings.fourAnimo = false;
    }
  } catch (_) {}
...
export const matchGenre = (categoryNameOrSlug: string, animeGenres: any): boolean => {
  ...
  if (parsed.length > 0) {
    return parsed.some(genre => {
      ...
      if (normCategory.includes(normGenre) || normGenre.includes(normCategory)) return true;
...
export function filterAndDeduplicateAnimes(animes: any[]): any[] {
  ...
  const filteredAnimes = animes.filter(anime => {
    ...
    const source = isImported ? 'imported' : (anime.source === 'my_database' || idStr.startsWith('custom-') ? 'my_database' : 'four_animo');
...
export const api = {
  _homeInternal: async () => {
    ...
    let trendingList = [];
    if (hasCustomUploads) {
      const customTrending = getCustomLocal('trending');
      trendingList = filterAndDeduplicateAnimes(customTrending.map(mapAnime));
    } else if (globalSettings.fourAnimo && liveData?.trending) {
      trendingList = filterAndDeduplicateAnimes(liveData.trending.map(mapAnime));
      if (trendingList.length === 0) trendingList = fallbackAnimes.slice(0, 5).map(mapAnime);
    }

    let mostPopularList = [];
    if (hasCustomUploads) {
      const customPopular = getCustomLocal('popular');
      mostPopularList = filterAndDeduplicateAnimes(customPopular.map(mapAnime));
    } else if (globalSettings.fourAnimo && liveData?.mostPopular) {
      mostPopularList = filterAndDeduplicateAnimes(liveData.mostPopular.map(mapAnime));
      if (mostPopularList.length === 0) mostPopularList = fallbackAnimes.slice(3, 11).map(mapAnime);
    }

    let newAddedList = [];
    if (hasCustomUploads) {
      const customNew = getCustomLocal('recentlyAdded');
      newAddedList = filterAndDeduplicateAnimes(customNew.map(mapAnime));
    } else if (globalSettings.fourAnimo && liveData?.newAdded) {
      newAddedList = filterAndDeduplicateAnimes(liveData.newAdded.map(mapAnime));
      if (newAddedList.length === 0) newAddedList = fallbackAnimes.slice(5, 10).map(mapAnime);
    }

    let topAiringList = [];
    if (hasCustomUploads) {
      const customAiring = getCustomLocal('topAiring');
      topAiringList = filterAndDeduplicateAnimes(customAiring.map(mapAnime));
    } else if (globalSettings.fourAnimo && liveData?.topAiring?.all) {
      topAiringList = filterAndDeduplicateAnimes(liveData.topAiring.all.map(mapAnime));
      if (topAiringList.length === 0) topAiringList = fallbackAnimes.slice(2, 10).map(mapAnime);
    }

    let latestEpisodeList = [];
    if (hasCustomUploads) {
      const customLatest = getCustomLocal('latest');
      latestEpisodeList = filterAndDeduplicateAnimes(customLatest.map(mapAnime));
    } else if (globalSettings.fourAnimo && liveData?.latestEpisode) {
      latestEpisodeList = filterAndDeduplicateAnimes(liveData.latestEpisode.map(mapAnime));
      if (latestEpisodeList.length === 0) latestEpisodeList = fallbackAnimes.slice(4, 12).map(mapAnime);
    }

    let completedAnimesList = [];
    if (hasCustomUploads) {
      const customCompleted = getCustomLocal('completed');
      completedAnimesList = filterAndDeduplicateAnimes(customCompleted.map(mapAnime));
    } else if (globalSettings.fourAnimo && liveData?.completedAnimes) {
      completedAnimesList = filterAndDeduplicateAnimes(liveData.completedAnimes.map(mapAnime));
      if (completedAnimesList.length === 0) completedAnimesList = fallbackAnimes.filter(a => a.status === 'Completed').map(mapAnime);
    }

    let topUpcomingAnimesList = [];
    if (hasCustomUploads) {
      const customUpcoming = getCustomLocal('upcoming');
      topUpcomingAnimesList = filterAndDeduplicateAnimes(customUpcoming.map(mapAnime));
    } else if (globalSettings.fourAnimo && liveData?.topUpcomingAnimes) {
      topUpcomingAnimesList = filterAndDeduplicateAnimes(liveData.topUpcomingAnimes.map(mapAnime));
      if (topUpcomingAnimesList.length === 0) topUpcomingAnimesList = fallbackAnimes.filter(a => a.status === 'Ongoing').slice(0, 8).map(mapAnime);
    }

    let mostFavoriteAnimesList = [];
    if (hasCustomUploads) {
      const customFavorite = getCustomLocal('favorite');
      mostFavoriteAnimesList = filterAndDeduplicateAnimes(customFavorite.map(mapAnime));
    } else if (globalSettings.fourAnimo && liveData?.mostFavoriteAnimes) {
      mostFavoriteAnimesList = filterAndDeduplicateAnimes(liveData.mostFavoriteAnimes.map(mapAnime));
      if (mostFavoriteAnimesList.length === 0) mostFavoriteAnimesList = fallbackAnimes.slice(1, 9).map(mapAnime);
    }
      ...
      dynamicSections: dynamicSections.map(sec => {
        ...
        if (isMatch('trending') || isMatch('trendinganime')) {
          ...
          if (customHindi.length > 0) {
            ...
            sectionAnimes = hindiUploads.length > 0 ? hindiUploads : [];
          } else if (globalSettings.fourAnimo) {
          ...
          if (customOngoing.length > 0) {
            ...
            sectionAnimes = ongoingUploads.length > 0 ? ongoingUploads : [];
          } else if (globalSettings.fourAnimo) {
          ...
          if (customMovies.length > 0) {
            ...
            sectionAnimes = movieUploads.length > 0 ? movieUploads : [];
          } else if (globalSettings.fourAnimo) {
  ...
  category: async (categorySlug: string, categoryName?: string): Promise<Anime[]> => {
    ...
    // 2. If custom is empty, check fallbackAnimes matching this genre/category ONLY if 4Animo integration is enabled
    if (globalSettings.fourAnimo) {
      const searchKw = categoryName || categorySlug;

      const matchedFallback = fallbackAnimes.filter(a => {
        if (normSlug === 'movie' || normSlug === 'movies') {
          return isMovieItem(mapAnime(a));
        }
        return matchGenre(searchKw, parseAnimeGenres(a.genres)) || matchGenre(categorySlug, parseAnimeGenres(a.genres));
      });
  ...
  search: async (keyword: string, page = 1, filters: { type?: string; status?: string; season?: string; year?: string } = {}) => {
    ...
    // 2. High-priority static anime dataset & Live API search ONLY if fourAnimo is enabled
    if (globalSettings.fourAnimo) {
