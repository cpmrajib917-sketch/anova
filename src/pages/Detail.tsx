...
import { api, apiCache, fallbackAnimes, prefetchAnime } from '../lib/api';
...
export function Detail() {
  ...
  // Function to format raw or numeric titles cleanly
  const formatAnimeTitle = (rawTitle: any, animeId: any) => {
    if (!rawTitle) return `Anime Series #${String(animeId).replace(/^custom-/i, '')}`;
    const cleaned = cleanAnimeTitleForDisplay(String(rawTitle));
    // Check if the title is purely digits or raw custom ID
    if (/^\d+$/.test(cleaned) || /^custom-\d+$/i.test(rawTitle)) {
      return `Anime Series #${String(animeId || rawTitle).replace(/^custom-/i, '')}`;
    }
    return cleaned;
  };
  ...
  useEffect(() => {
    ...
    api.animeInfo(id).then((res) => {
      ...
      if (res) {
        setAnime({ ...res, title: formatAnimeTitle(res.title, id) });
      }
    }).catch((err) => {
      ...
      <div className="relative min-h-[460px] md:min-h-[500px] w-full overflow-hidden border-b border-white/5 flex flex-col justify-between pt-16 md:pt-20">
        ...
        <div className="absolute inset-0 pointer-events-none z-0">
          <img 
            ...
            className="w-full h-full object-cover object-[center_20%] opacity-35 md:opacity-25" 
            onError={(e) => {
        ...
        <div className="relative z-20 w-full px-4 md:px-8 max-w-7xl mx-auto pb-6 mt-auto">
          <div className="flex flex-col sm:flex-row items-center sm:items-end gap-5 text-center sm:text-left">
            ...
            <img
              ...
              alt={anime.title}
              referrerPolicy="no-referrer"
              className="w-32 sm:w-36 md:w-44 aspect-[2/3] object-cover rounded-2xl shadow-[0_8px_30px_rgba(0,0,0,0.8)] border border-white/10 shrink-0"
              onError={(e) => {
