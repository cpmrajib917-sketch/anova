...
export function AnimeCard({ anime, className }: AnimeCardProps) {
  ...
  const displayTitle = (() => {
    ...
    if (/^\d+$/.test(clean) || /^custom-\d+$/i.test(anime.title)) {
      return `Anime #${String(anime.id || anime.title).replace(/^custom-/i, '')}`;
    }
    return clean;
  })();
  ...
  return (
    <Link
      ...
      onClick={handleClick}
      className={cn(
        "group block relative rounded-xl overflow-hidden bg-[#0a0d14]/80 hover:bg-[#0e1320] border border-white/5 shadow-lg transition-transform duration-150 ease-out hover:scale-[1.02] hover:border-primary/30 touch-action-manipulation",
        className
      )}
    >
      ...
      <div className="aspect-[9/14] relative overflow-hidden bg-[#03060c] select-none">
        <img
          ...
          className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-103 bg-white/5"
          onError={(e) => {
        ...
        <div className="absolute top-2 left-2 right-2 flex flex-wrap justify-between items-start gap-1 pointer-events-none">
          ...
          {anime.rating && (
            <span className="bg-black/85 text-yellow-400 text-[8px] md:text-[9px] font-black px-1.5 py-0.5 rounded shadow-[0_2px_4px_rgba(0,0,0,0.5)] flex items-center gap-0.5 shrink-0">
              ...
              {typeof anime.rating === 'number' || (!isNaN(Number(anime.rating)) && String(anime.rating).trim() !== '')
